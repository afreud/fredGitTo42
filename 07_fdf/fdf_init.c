...
static void	ft_init_map(char *path, t_xdata *xdata)
{
	...
	if ((*xdata).map.width <= 1 || (*xdata).map.height <= 1)
...
static void	ft_init_mlx(char *path, t_xdata *xdata)
{
	...
	if (!(*xdata).img.mlx_img)
		ft_mlxexit(xdata, errno, NULL);
	(*xdata).img.addr = mlx_get_data_addr((*xdata).img.mlx_img,
			&(*xdata).img.bpp, &(*xdata).img.line_len, &(*xdata).img.endian);
	if (!(*xdata).img.addr)
		ft_mlxexit(xdata, errno, NULL);
}
